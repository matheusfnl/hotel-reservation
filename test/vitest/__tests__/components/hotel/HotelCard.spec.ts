...
import { QBtn, QRating } from 'quasar'
...
describe('HotelCard', () => {
  ...
  it('should show hotel name correctly', async () => {
    const wrapper = mountWrapper()
  ...
  it('should show refundable chip when hotel is refundable', () => {
    const wrapper = mountWrapper()
    const chip = wrapper.find('.q-chip')

    expect(chip.exists()).toBe(true)
    expect(chip.text()).toContain(wrapper.vm.$t('hotel.hotel.refundable'))
  })
})
